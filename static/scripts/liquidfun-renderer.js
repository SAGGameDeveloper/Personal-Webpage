// Original source: https://github.com/doebi/liquidfun.js-demo

function highest2(x) {
    return Math.pow(2, Math.ceil(Math.log(x) / Math.LN2));
};

function LiquidfunSprite(particleSystem, renderer) {
    ...
    this.ball_shader = new PIXI.Shader(renderer.renderer.gl, ball_vert, ball_frag);
    this.blur_shader = new PIXI.Shader(renderer.renderer.gl, identity_vert, blur_frag);
    this.threshold_shader = new PIXI.Shader(renderer.renderer.gl, identity_vert, threshold_frag);
...
function LiquidfunRenderer(renderer, PTM) {
...
PIXI.Renderer.registerPlugin('liquidfun', LiquidfunRenderer);
...
LiquidfunRenderer.prototype.render = function (sprite) {
    ...
    if (count > 0) {
        ...
        raw_pos = new Float32Array(Module.HEAPU8.buffer, pos_offset.e, count * 2);
        ...
        for (let i = 0; i < count; i++) {
            position[i*2]   = (raw_pos[i*2]   - fx) * 2 * this.PTM / w;
            position[i*2+1] = (raw_pos[i*2+1] - fy) * 2 * this.PTM / h;
        }
        // upload data to gpu
        ...
        sprite.ball_shader.uniforms.size = radius * this.PTM * this.blurRadius;
